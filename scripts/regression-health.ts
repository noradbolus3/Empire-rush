...
assert.equal((retailTick.businesses[0] as typeof retail).stockUnits, 8, 'retail: stock deduction failed');
assert.equal(retailTick.cashDelta, 5.35, 'retail: sale revenue/tax settlement failed');
...
const ipoBusiness = { ...retail, baseHourlyNetProfit: 25000, hourlyNetProfit: 25000 };
assert.equal(getIPOEligibility({ ...ipoBusiness, isAcquired: false }, null, 10_000_000).eligible, false, 'IPO: unacquired company was eligible');
assert.equal(getIPOEligibility(ipoBusiness, null, 10_000_000).eligible, true, 'IPO: eligible company was blocked');
const listing = createIPOListing(ipoBusiness, Date.UTC(2026, 0, 1), 'IPO', 10_000_000);
assert.equal(getIPOEligibility(ipoBusiness, listing, 10_000_000).eligible, false, 'IPO: duplicate public offering was allowed');
const cappedListing = capIPOProceeds(createIPOListing(ipoBusiness, Date.UTC(2026, 0, 1), 'IPO', 1_000_000), 1_000_000);
assert.equal(founderOwnershipFraction(cappedListing), 0.8, 'IPO: founder ownership was not diluted to 80%');
assert(cappedListing.capitalRaised <= 500_000, 'IPO: proceeds exceeded 0.5x net worth cap');
const valuationAt10M = businessValuation(ipoBusiness, 10_000_000);
const valuationAt50M = businessValuation(ipoBusiness, 50_000_000);
assert(valuationAt10M <= 40_000_000 && valuationAt50M <= 200_000_000, 'IPO: valuation exceeded 4x net-worth cap');
assert(valuationAt50M > valuationAt10M, 'IPO: larger net-worth company valuation order inverted');
const legacyListing = capIPOProceeds({ ...listing, valuationAtIPO: 300_000_000, currentPrice: 300, ipoPrice: 300, capitalRaised: 23_000_000, history: [288, 294, 300] }, 25_000_000);
assert.equal(legacyListing.valuationAtIPO, 100_000_000, 'IPO: legacy valuation was not normalized to 4x net worth');
assert(legacyListing.currentPrice < 100, 'IPO: legacy share price was not normalized with valuation');
assert(legacyListing.capitalRaised <= 12_500_000, 'IPO: legacy proceeds exceeded 0.5x net worth');
...
assert.match(read('src/components/modals/IPOAnnouncementModal.tsx'), /PUBLIC COMPANY FOUNDER/, 'IPO: milestone celebration missing');
assert.match(app, /first-ipo/, 'IPO: milestone achievement was not persisted');
assert.match(app, /Starter Stock Credit/, 'monetization: direct cash IAP shortcut was not removed');
assert.doesNotMatch(app, /Billionaire Club/, 'monetization: billionaire cash shortcut remains');
...
assert(tapUpgradeCost(17) >= 5000 && tapUpgradeCost(17) <= 6200, 'tap ROI: level 17 cost left the corrected ~$5.8K range');
assert.match(read('scripts/ipo-economy-round7.ts'), /maxStepMultiple/, 'IPO: smooth-curve simulation missing');
assert.match(read('scripts/ipo-valuation-round9.ts'), /valuationToNetWorth/, 'IPO: structural valuation proof missing');
assert.match(app, /const safeListing = capIPOProceeds\(ipoListing, netWorth\)/, 'IPO: hydrated legacy listing normalization missing');
