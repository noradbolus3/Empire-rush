...
orderStock(1000, 2000);
for (let elapsed = 2; elapsed <= 24 * 60 * 60; elapsed += 2) {
  if (!autoRestock && cash >= 1500 && elapsed <= 15 * 60) {
    cash -= 1500;
    autoRestock = true;
  ...
  const sold = Math.max(0, Math.round((business.lastSaleRevenue ?? 0) / ({ Discount: 2.8, Standard: 3.8, Luxury: 6.5 } as const)[business.pricingTier]));
  salesUnits += sold;
  ...
  if (label) snapshots.push({ checkpoint: label, orders, salesUnits, cashflow: Number(cashflow.toFixed(2)), cash: Number(cash.toFixed(2)), netWorth: Number(netWorth().toFixed(2)), minNetWorth: Number(minNetWorth.toFixed(2)), autoRestock, demandEvents, flag: minNetWorth < 5000 ? 'FAIL: net worth fell' : 'PASS' });
}
console.log(JSON.stringify({ startingCash: 5000, initialBusinessPurchase: retail.acquisitionCost, initialOrder: '$2,000 for 1,000 units', snapshots }, null, 2));
