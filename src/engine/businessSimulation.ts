...
export function passiveIncomeMultiplier(netWorth: number): number { return Number(Math.min(8, Math.pow(1 + Math.max(0, Number.isFinite(netWorth) ? netWorth : 0) / 5000, 0.3)).toFixed(4)); }
export const DEFAULT_BUSINESSES: BusinessEntity[] = [
  { id: 'apex-retail', name: 'Copper & Bloom Market', sector: 'Retail', isUnlocked: true, unlockNetWorthRequired: 0, isAcquired: false, acquisitionCost: 2500, legalStatus: 'Licensed_Legal', policeHeat: 0, stability: 92, hourlyNetProfit: 0, stockUnits: 0, maxStockCapacity: 2500, pricingTier: 'Standard', hasSecurity: false, hasManager: false, onboardingStep: 'ORDER_STOCK', unitWholesaleCost: 2, monthlyRent: 800, monthlyPayroll: 1200, salesRemainder: 0, autoRestockEnabled: false, demandEvent: 'None', demandEventSeconds: 0, demandClockSeconds: 0, pulseChain: 0, lastSaleSequence: 0, lastSaleRevenue: 0 },
  { id: 'metro-mobility', name: 'Neon Mile Fleet', sector: 'Mobility', isUnlocked: false, unlockNetWorthRequired: 25000, isAcquired: false, acquisitionCost: 25000, legalStatus: 'Licensed_Legal', policeHeat: 0, stability: 90, hourlyNetProfit: 0, economySedans: 10, electricEVs: 0, luxuryLimos: 0, fleetHealth: 100, surgeActive: false },
  { id: 'cyberpulse-saas', name: 'SignalForge Cloud', sector: 'Tech_SaaS', isUnlocked: false, unlockNetWorthRequired: 100000, isAcquired: false, acquisitionCost: 100000, legalStatus: 'Licensed_Legal', policeHeat: 0, stability: 88, hourlyNetProfit: 0, activeSubscribers: 0, serverCapacity: 25000, openBugs: 0 },
...
function retailTick(business: RetailData, seconds: number, events: string[], liquidCash: number): { business: RetailData; cashDelta: number } {
  ...
  const restockCost = 2000;
  const canAutoRestock = Boolean(business.autoRestockEnabled) && business.stockUnits < business.maxStockCapacity * 0.15 && business.stockUnits + 1000 <= business.maxStockCapacity && liquidCash >= restockCost;
  const restockUnits = canAutoRestock ? 1000 : 0;
  ...
  const netCashDelta = Number(((unitsDeducted > 0 ? revenueEarned - fixedCosts - tax : 0) - (canAutoRestock ? restockCost : 0)).toFixed(2));
  ...
  if (canAutoRestock) events.push(`${business.name}: Shelf Runner auto-ordered 1,000 units for $2,000.`);
...
export function simulateBusinessTick(businesses: BusinessEntity[], seconds = 2, constructionElapsedSeconds = seconds, liquidCash = Number.MAX_SAFE_INTEGER, ownershipFractions: Record<string, number> = {}, playerNetWorth = 0): SimulationResult {
  ...
  const acquiredCount = businesses.filter(item => item.isAcquired).length; const wealthMultiplier = passiveIncomeMultiplier(playerNetWorth); const next = businesses.map(item => { const result = item.sector === 'Retail' ? applySynergy(retailTick(item, seconds, events, liquidCash), acquiredCount) : item.sector === 'Mobility' ? applySynergy(mobilityTick(item, seconds), acquiredCount) : item.sector === 'Tech_SaaS' ? applySynergy(saasTick(item, seconds, events), acquiredCount) : item.sector === 'Construction_Mega' ? applySynergy(constructionTick(item, constructionElapsedSeconds, events), acquiredCount) : applySynergy(expansionTick(item, seconds, events), acquiredCount); const founderShare = Math.min(1, Math.max(0, Number(ownershipFractions[item.id] ?? 1))); cashDelta += result.cashDelta * founderShare * wealthMultiplier; return { ...result.business, hourlyNetProfit: Number((result.business.hourlyNetProfit * founderShare * wealthMultiplier).toFixed(2)) }; });
  return { businesses: next, cashDelta: Number(cashDelta.toFixed(2)), events };
}

const normalizeBusinessNames = (businesses: BusinessEntity[]): BusinessEntity[] => businesses.map(item => item.id === 'brightgrid-energy' || (item.sector === 'Energy' && ['BrightGrid Energy', 'Surya Energy'].includes(item.name)) ? { ...item, name: 'Sunward Gridworks' } : item);
