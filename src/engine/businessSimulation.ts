...
const normalizeBusinessNames = (businesses: BusinessEntity[]): BusinessEntity[] => businesses.map(item => item.id === 'brightgrid-energy' || (item.sector === 'Energy' && ['BrightGrid Energy', 'Surya Energy'].includes(item.name)) ? { ...item, name: 'SunPeak Energy' } : item);
